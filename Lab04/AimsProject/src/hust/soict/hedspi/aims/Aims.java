...
public class Aims {
    ...
    private static void seedSampleData(Store store) {
        ...
        cd1.addTrack(new Track("Open Road", 4));
        cd1.addTrack(new Track("Night Drive", 5));
    ...
    private static void showMediaDetails(Store store, Cart cart, Scanner scanner) {
        ...
        if (choice == 1) {
            cart.addMedia(media);
            // If added media is a DVD, show number of DVDs in cart
            if (media instanceof DigitalVideoDisc) {
                int dvdCount = 0;
                for (Media m : cart.getItemsOrdered()) {
                    if (m instanceof DigitalVideoDisc) dvdCount++;
                }
                System.out.println("Number of DVDs in cart: " + dvdCount);
            }
        } else if (choice == 2 && playable) {
            playMedia(media);
        }
    }
